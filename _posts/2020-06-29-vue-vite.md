...
How does this work for importing packages? Well, there is no difference with the process described in A 
...
But there is an important point: this only works **provided that the package has a distribution in ES format**. If not, Vite will just not find the module and return an error on the request for the module exported by the package.
...
Vite also accepts custom plugins, in case one needs specific code transformations.
