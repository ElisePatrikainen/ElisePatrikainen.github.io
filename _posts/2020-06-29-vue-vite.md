...
Then, when the whole dependency graph has been loaded, modules are **instantiated** (from the most nested to the top), their exports are made available, and only then **the most up code** will be executed.
...
const b = cloneDeep(a);
```
Then Vite throws an error saying that it cannot find the export.
...
Anyways, hoefully ES distributions shall become one day the norm standard, so this problem will resolve by itself at some point.
