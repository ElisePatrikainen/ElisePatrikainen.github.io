...
Vite also supports HMR (Hot Module Reload) for Vue, React, Svelte. So, this combined with 'unbundled development' (term coined by [@FredKSchott](todo link)) enables Vite to instantaneously update a running app with any code changes, and to **free the front-end developer from the wait for compilation**.

> Nb: Vite means 'fast' in French [drapeau]

So how does this technological wonder works?

<span style="color: red; font-style: italic">[Note: a consistent part of this article gives an overview of Vite's concepts and how Vite works, but is not stricly relevant to use Vite. **The user in a hurry can go directly to the parts II and IV**, which present Vite's main features and gives some hints on how to "migrate" to Vite.]</span>
...
> Here is a reminder on the most important **differences between modules and regular script** (this is not exhaustive):
 ...
 - `this` refer to `undefined` (instead of the global `this`)
...
This raises a question: **will Vite choose to support other formats than ESM** (// todo: check this: as snowpack does)? According to this [issue](https://github.com/vitejs/vite-plugin-react/issues/15#issuecomment-661578986), it will not, because it would require to implement a dubious solution.

Indeed, a non-ES module may rely on some dynamic logic which is difficult for ESM bundlers (such as Rollup) to analyze. Therefore,an ESM bundler can be transpille a module into ESM (for Rollup, with `@rollup/plugin-commonjs`), but it will transpille it into a single export
...
The idea is that these tasks can be handled by other tools (IDEs, tests librairies...) and setting them in Vite would be redundant. 
...
As we mentionned in II., Vite performs code compilation on demand, with features inspired from Webpack loaders. 
...
Oberwise, Vite will shut down the process with this message:
...
This is discussed in II. D. and in IV. A. For testing, it is suggested to use directly testing libraries, whithout React utilities. Or to **wait for React to provide an ESM distribution**, which should happen in v. 17.
</li>
...
In production, Vite also offers a bundled build based on Rollup, which garanties higly optimised bundles (thanks to ESM support).

For now, Vite is still **experimental**, hence we are looking forward to future developments, and, perhaps, an integration in Vue or other framewoks standard configuration. But there is no doubt on one thing: **this will definitely improve front-end experience**.
