...
<span style="color: red; font-style: italic">[Diclaimer: the three first parts give an overview of Vite's concepts and how Vite works, but are absolutely not needed to use Vite. **For the user primarily interested in using Vite, please skip directly the part IV**, which gives some elements on how to "migrate" to Vite.]</span>



### I. [Reminder] ES modules in browsers? / core concepts:


#### A. Bundled vs unbundled development:

Here are two terms which have been coined / popularised (I think) by @FredKSchott. You can find [here](https://www.snowpack.dev/#unbundled-development) an accurate definition, which I summarize here:
...
``` 
And they also implement dynamic module loading:
...
Inited by SnowPack
Why Vite differs from SP (cf tweet)
...
#### C. External dependancies management:

How does this work for importing packages? Well, there is no difference with the process described in A **provided that the package has a distribution in ES format**. This is a very important point ; if not, Vite will just not find the module and return an error on the request for the module exported by the package.

// the error

This might be the most serious issue for migrating to Vite because, even if ES distributions will be at some point the norm, for now it is not and many packages do not provide it (cf I. B and III. A.). Will Vite choose to support other formats (as snowpack?)? Perhaps, but I don't think this is Vite's design (pure speculation). Plus, if they want to support the ES turn, they should not

Prod: Rollup supports other formats
...
In the example we presented in I., we have two types of imports: 
 - Statements importing ES modules 👍: the Vue distribution (`/@modules/vue`)
 > Vue has an ES distribution, but we will talk later about non ES dependancies management (cf C.)
 - Statements importing resources which **are not ES modules** 👎: `.vue` files, CSS stylesheet...

So how does Vite deal with that?

Well, it simply **transforms on the fly the required resource into valid modules** thanks to a "pipe" of plugins performing various code transformations (which to some extend reminds Webpack loaders). There are plugins for:
- resources: CSS, images...
- Vue files
- **TypeScript**, **JSX** and **CSS pre-processors**
- [additional] **React**, **Preact**
...
[optional] remove CLI packages:
<div class="highlighter-rouge"><div class="highlight"><pre class="highlight">
<code>yarn remove @vue/cli-plugin-babel @vue/cli-plugin-eslint @vue/cli-service
</code></pre></div></div>
...
<span class="changed-code added-code">+      "build": "vite build"</span>
    "lint": "vue-cli-service lint", // A garder ?
},
</code></pre></div></div>
</li>
...
<li>move if needed the `index.html` in the root folder (otherwise it may not work in build time)
</li>
